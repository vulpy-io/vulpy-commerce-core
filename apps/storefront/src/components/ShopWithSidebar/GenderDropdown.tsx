...
const GenderItem = ({ category }) => {
  ...
  return (
    ...
    >
      ...
      <span
        className={`${
          selected ? "bg-action-primary-background text-white" : "bg-surface-subtle"
...
const GenderDropdown = ({ genders }) => {
  ...
  return (
    <div className="rounded-lg bg-white shadow-1">
      <div
        className={`flex cursor-pointer items-center justify-between py-3 pr-5.5 pl-6 font-semibold ${
          toggleDropdown && "shadow-filter"
        }`}
        onClick={() => setToggleDropdown(!toggleDropdown)}
      >
        <p className="text-content-primary">Gender</p>
        <button
          aria-label="Expand gender filter"
      ...
      >
        {genders.map((gender, key) => (
          <GenderItem category={gender} key={key} />
        ))}
      </div>
    </div>
  );
