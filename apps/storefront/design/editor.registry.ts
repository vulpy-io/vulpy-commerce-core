/**
 * Design Editor Registry
 *
 * Exhaustive, operator-safe list of token paths Fox is allowed to modify.
 * Every entry maps a dot-path into the DTCG token tree to a human-readable
 * label, a type constraint, and optional Impeccable hint metadata.
 *
 * RULES FOR AGENTS
 * ─────────────────
 * 1. Only paths listed here may be written to store.tokens.json.
 * 2. Never write a value that fails the `allowed` guard.
 * 3. Never promote a reference path to a component path — the three-layer
 *    architecture (reference → semantic → component) is enforced by the
 *    compiler; edits should happen at the reference layer.
 * 4. When Impeccable recommends a value it must match a registered path;
 *    unregistered paths are silent no-ops in the apply pipeline.
 * 5. `dangerLevel` drives the diff-approval UX:
 *      low  → auto-apply (Fox can commit without showing diff)
 *      medium → show diff, single-click approve
 *      high → require explicit written approval from operator
 * 6. `scope` marks the surface boundary:
 *      brand → safe for brand adoption (marketing + commerce-display tokens)
 *      commerce-safe → affects conversion paths, extra care required
 */

export type TokenType = "color" | "fontFamily" | "fontWeight" | "dimension" | "number" | "string";

export type DangerLevel = "low" | "medium" | "high";

export type SurfaceScope = "brand" | "commerce-safe";

export interface TokenEntry {
  /** Dot-path into the DTCG token tree (matches store.tokens.json key path). */
  path: string;
  /** Human label shown in Payload Design Studio view. */
  label: string;
  /** Short helper text surfaced in the diff-approval UI. */
  hint: string;
  /** DTCG $type — validated at compile time. */
  type: TokenType;
  /** How risky is a change to this token? */
  dangerLevel: DangerLevel;
  /** Surface scope — brand or commerce-safe. */
  scope: SurfaceScope;
  /**
   * Optional allow-list of exact values (for enums) or a validation
   * function. When omitted any value passing the type check is accepted.
   */
  allowed?: string[] | ((v: string) => boolean);
  /**
   * When true this token is visible in the Payload operator UI.
   * Set false for tokens that are technically editable but should only
   * be touched by Fox / Impeccable, not a human clicking around.
   */
...
const isHexColor = (v: string) => HEX_COLOR_RE.test(v);

// ─── Registry ─────────────────────────────────────────────────────────────────

export const EDITOR_REGISTRY: TokenEntry[] = [
  // ── Brand palette ──────────────────────────────────────────────────────────
  {
...
/** Indexed lookup: path → entry. O(1) after first call. */
let _index: Map<string, TokenEntry> | null = null;
export function getRegistryIndex(): Map<string, TokenEntry> {
  if (!_index) {
...
export function isAllowedEdit(path: string, value: string): boolean {
...
export const OPERATOR_VISIBLE_PATHS: string[] = EDITOR_REGISTRY
  .filter((e) => e.operatorVisible !== false)
  .map((e) => e.path);

/** Surface scope helpers. */
export const BRAND_PATHS: string[] = EDITOR_REGISTRY
  .filter((e) => e.scope === "brand")
  .map((e) => e.path);

export const COMMERCE_SAFE_PATHS: string[] = EDITOR_REGISTRY
