...
- **For WebUI delivery: include `MEDIA:/absolute/path/to/file` in the response**
  so the file renders inline (images, video, PDF, HTML, diffs, CSV all supported).
  Do NOT hardcode private repo URLs, localhost ports, or server-absolute paths in
  the artifact content — the user sees the file through the WebUI, not through a
  server URL.
- **Never put private-repo GitHub URLs in artifact content or output.** If the
  artifact lives in a private repo, reference it by its local file path or use
  MEDIA delivery. A GitHub blob URL to a private repo is inaccessible to anyone
  without access and leaks internal paths.
...
- artifact path (include `MEDIA:/absolute/path` for WebUI delivery)
- what it contains
- verification status
