...
- Tell the user how to access the sketches:
  - **Hermes WebUI:** include `MEDIA:/absolute/path/to/file` so HTML files render inline
  - Local open: `open sketches/001-calm-editorial/index.html` on macOS, `xdg-open` on Linux, `start` on Windows
- Never hardcode private-repo GitHub URLs or localhost ports in artifact output — use MEDIA delivery or local paths
