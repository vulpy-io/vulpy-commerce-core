...
**`agent.preload_skills` is NOT a valid config key** — it does not exist in
`DEFAULT_CONFIG` and is silently ignored. Adding it to a profile's
`config.yaml` will have zero effect. To pre-load skills for a profile, use
wrapper scripts or the `-s` flag explicitly:

```bash
# wrapper approach (recommended): create ~/.local/bin/hermes-<role>
exec hermes -p myprofile -s skill1,skill2 "$@"

# one-off
hermes -p myprofile -s skill1,skill2 "Do the thing"
```

**Sharing mem0 across profiles** — each profile gets its own `HERMES_HOME`
(`~/.hermes/profiles/<name>/`), so by default every profile has an isolated
`mem0_oss/` store. To make all profiles read/write the same memory corpus,
write a `mem0_oss.json` into each profile directory pointing both paths at
the root store:

```json
{
  "vector_store_path": "/path/to/.hermes/mem0_oss/qdrant",
  "history_db_path": "/path/to/.hermes/mem0_oss/history.db"
}
```

`mem0` reads `$HERMES_HOME/mem0_oss.json` at startup and uses it as the
config override. This is fully reversible — delete the file to restore
profile isolation. The `MEM0_OSS_VECTOR_STORE_PATH` and
`MEM0_OSS_HISTORY_DB_PATH` env vars do the same thing per-invocation.
