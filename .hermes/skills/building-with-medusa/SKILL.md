...
description: "Load automatically when you plan, research, or implement Medusa backend features: custom modules, API routes, workflows, data models, module links, business logic. Required for all Medusa backend work in all modes. Contains architectural patterns and critical rules that MCP servers don't provide."
---

# Medusa Backend Development

Backend development guide for Medusa applications. Contains patterns across 6 categories: architecture, type safety, business logic placement, and common pitfalls.
...
**The quick reference below is not sufficient for implementation.** You must load relevant reference files before you write code for that component.
...
## Dev/Admin proxy pitfalls

- Medusa Admin Vite HMR behind Fox Tailscale/public edge can leak Vite's random internal websocket port (for example `wss://<host>:40155/app`). See `references/medusa-admin-vite-hmr-proxy.md` before treating this as a CSP/Caddy iframe issue. For mixed public-edge + Tailscale access, disable Admin HMR by default with a local opt-in (`MEDUSA_ADMIN_HMR=1`) rather than hardcoding one global `clientPort`.

## Validating Implementation

**CRITICAL: Always run the build command after you complete implementation. It catches type errors and runtime issues.**
...
4. Do not mark implementation as complete until the build succeeds
...
## Verifying installed Medusa capabilities (version-grounded answers)

When asked "does Medusa support X?" or "what's missing for feature Y" (client scoping, gap analysis, feasibility), probe the installed package — docs trail versions and the pnpm tree is right there.

- Medusa core in this workspace: `apps/medusa-backend/node_modules/@medusajs/medusa` (root `node_modules/@medusajs/medusa` does NOT exist — pnpm workspace).
- Core modules are barrels: `dist/modules/inventory.js` just re-exports `@medusajs/inventory` (locate with `ls -d node_modules/.pnpm/@medusajs+inventory@*/node_modules/@medusajs/inventory`).
- Bundled dist JS is minified — grepping for `async method(` fails. Read the `.d.ts` surface instead: `grep -oE "^    [a-zA-Z]+\(" <service>.d.ts | sort -u`.
- Admin API surface: `ls <medusa>/dist/api/admin/` filtered by domain (inventory|reservation|stock|location|fulfillment|order).
- Model list: `ls <module>/dist/models/` — quick way to confirm absent entities (e.g. no supplier, no purchase-order).

Inventory capability map for 2.13.0 (what exists, what's absent, build-vs-buy for client scoping): see `references/inventory-capability-map.md`.

## When to Use This Skill vs MedusaDocs MCP Server

**⚠️ CRITICAL: Consult this skill first for planning and implementation.**
