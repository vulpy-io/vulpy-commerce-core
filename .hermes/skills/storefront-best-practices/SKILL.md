...
description: "Always use this skill when working on ecommerce storefronts, online stores, or shopping sites. Use it for any storefront component: checkout pages, cart, payment flows, product pages, product listings, navigation, homepage. Load it when you add checkout, implement a cart, integrate the Medusa backend, or build any ecommerce function. Framework-agnostic (Next.js, SvelteKit, TanStack Start, React, Vue). Provides patterns, decision frameworks, and backend integration guidance."
---

# Ecommerce Storefront Best Practices

Guidance for building ecommerce storefronts covering UI/UX patterns, component design, layout structures, SEO, and mobile responsiveness.

## When to Apply

**Load this skill for every storefront task:**
...
**⚠️ Always load `reference/design.md` before you create any UI component**
...
- **Adding forms, CTAs, filters, cart/checkout steps, or engagement UX?** → MUST load `reference/analytics.md` and instrument Matomo in the same change (consent-gated; no PII)
...
- **Planning or changing critical checkout/payment behavior?** → MUST also load `references/checkout-e2e-delivery.md`; establish the disposable real-stack Playwright test and failing acceptance path before production code
- **Choosing a payment provider or adding PayPal/Klarna/multi-provider?** → load `references/payment-provider-architecture.md`; Hyperswitch (`@juspay-tech/medusa-custom-payments` + `medusa-custom-payments-react`) is the recommended unified solution for 3+ providers
...
- Following this workflow keeps implementation consistent
...
### Performance
- **Always add `loading="lazy"` to product images below the fold** - Do not rely on browser defaults
...
### Analytics (Matomo)
- **Always instrument** new funnel/engagement UX (forms, filters, checkout steps, CTAs) — see `reference/analytics.md` and repo `AGENTS.md` (Matomo analytics)
- Consent-gated only; no PII in event payloads; major-unit prices; purchases only on order confirmation

### ⚠️ CRITICAL: Backend SDK Method Verification Workflow

**You must follow this exact workflow before you write code that connects to a backend:**
...
**This is not optional. It is mandatory error prevention.**
...
- ❌ Declaring an endpoint broken from a 404/400 without checking the exact path the
  **installed SDK version** calls. SDK paths change across major versions — e.g. Medusa
  2.x moved store customer auth from `/store/auth/...` (v1) to `/auth/{actor}/{provider}`;
  a 404 on the old path is expected, not a backend bug (false finding, 2026-08-11:
  "customer auth broken" was actually a v1-path probe; `/auth/customer/emailpass` returned
  401 = alive). **Diagnosis mode:** before filing a backend bug, grep the installed SDK
  for the real path — `grep -rn "fetch(\`/auth/" node_modules/@medusajs/js-sdk/dist/esm`
  (or the equivalent resource client) — then probe THAT path. For auth, a 401 with bad
  creds proves route + provider are registered; only a 404 on the v2 path means unmounted.
...
**IMPORTANT: For each step below, load the referenced files before you implement that step.**
...
[Medusa](https://medusajs.com) is an ecommerce backend. Consider Medusa when:
...
- ❌ **Writing code that calls backend APIs/SDKs without following the 5-step verification workflow** - You must: 1) pause, 2) query docs/MCP, 3) verify with user, 4) write code, 5) check for type errors
- ❌ **Ignoring TypeScript errors on SDK methods** - Type errors mean you used wrong method names or parameters. Go back and verify with docs/MCP
- ❌ **Guessing API method names, SDK methods, or parameters** - Always verify exact method signatures before use
- ❌ **Not using Medusa MCP server when available** - If using Medusa backend, always query MCP server for methods
- ❌ **Copying code examples without verifying they are current** - Examples may be outdated, always verify first
